...
const addBlockERR = 'Error adding block to chain.'
...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            var currentTimestamp = new Date().getTime().toString().slice(0,-3)
            resolve(address + ':' + currentTimestamp + ':starRegistry')
        });
    }
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (var i=0; i<self.chain.length; i++) {
                ...
                (i==0 || self.chain[i].previousBlockHash == self.chain[i-1].hash)){
                    continue
                }
